...
public class MuPDFCore {
	...
	public MuPDFCore(Context context, String filename) throws Exception {
		...
		if (globals == 0) {
			try {
				throw new Exception(
						String.format(context.getString(TiRHelper
								.getResource("string.cannot_open_file_Path")),
								filename));
	...
	public MuPDFCore(Context context, byte buffer[], String magic)
		...
		if (globals == 0) {
			try {
				throw new Exception(context.getString(TiRHelper
						.getResource("string.cannot_open_buffer")));
			} catch (ResourceNotFoundException e) {
			}
		}
		file_format = fileFormatInternal();
