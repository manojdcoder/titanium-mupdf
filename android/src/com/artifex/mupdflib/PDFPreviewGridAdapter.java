...
public class PDFPreviewGridAdapter extends BaseAdapter {
	...
	private static final String TAG = PDFPreviewGridAdapter.class
			.getSimpleName();
	...
	public PDFPreviewGridAdapter(Context context, MuPDFCore core, int position) {
		...
		File documentCache = new File(StorageUtils.getCacheSubDirectory(
				mContext, "previews2"), MD5.MD5Hash((new File(core
				.getFileName())).getName()));
		...
		try {
			mLoadingBitmap = BitmapFactory.decodeResource(
					...
					TiRHelper.getResource("drawable.darkdenim3"));
		} catch (ResourceNotFoundException exp) {
			Log.e("MuPDFPreviewGridAdapter", "XML resouce not found!");
		}
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			try {
				convertView = inflater.inflate(
						TiRHelper.getResource("layout.preview_grid_item"),
						parent, false);
			} catch (ResourceNotFoundException exp) {
				Log.e("PDFPreviewGridAdapter", "XML resouce not found!");
			}
		...
		holder.previewGridItemRelativeLayout
		...
		ViewHolder(View view) {
			try {
				this.previewPageImageView = (ImageView) view
						.findViewById(TiRHelper
								.getResource("id.preview_grid_image"));
				this.previewPageProgress = (ProgressBar) view
						.findViewById(TiRHelper
								.getResource("id.preview_grid_image_progressbar"));
				// this.previewPageNumber = (TextView)
				// view.findViewById(R.id.PreviewPageNumber);
				this.previewGridItemRelativeLayout = (RelativeLayout) view
						.findViewById(TiRHelper
								.getResource("id.PreviewGridItemRelativeLayout"));
			} catch (ResourceNotFoundException exp) {
				Log.e("PDFPreviewGridAdapter", "XML resouce not found!");
			}
		}
			...
			if (mPreviewSize == null) {
				...
				try {
					padding = mContext.getResources().getDimensionPixelSize(
							TiRHelper.getResource("dimen.preview_height"));
				} catch (ResourceNotFoundException exp) {
					Log.e("PDFPreviewGridAdapter", "XML resouce not found!");
				}
							...
							try {
								holder.previewGridItemRelativeLayout
										.setBackgroundColor(mContext
												...
												.getColor(
														TiRHelper
																.getResource("color.thumbnail_selected_background")));
							} catch (ResourceNotFoundException exp) {
								Log.e("PDFPreviewGridAdapter",
										"XML resouce not found!");
							}
						} else {
							holder.previewGridItemRelativeLayout
									.setBackgroundColor(Color.TRANSPARENT);
	...
	private Bitmap getCachedBitmap(int position) {
		...
		if (lq == null) {
			...
			try {
				lq.compress(CompressFormat.JPEG, 70, new FileOutputStream(
						mCachedBitmapFile));
