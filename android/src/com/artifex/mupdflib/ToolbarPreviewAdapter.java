...
public class ToolbarPreviewAdapter extends BaseAdapter {
	...
	private static final String TAG = ToolbarPreviewAdapter.class
			.getSimpleName();
	...
	public ToolbarPreviewAdapter(Context context, MuPDFCore core) {
		...
		File documentCache = new File(StorageUtils.getCacheSubDirectory(
				mContext, "previews"),
				MD5.MD5Hash((new File(core.getFileName())).getName()));
		...
		try {
			...
			Log.e("ToolbarPreviewAdapter", "XML resouce not found!");
		}
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			try {
				convertView = inflater.inflate(TiRHelper
						.getResource("layout.preview_pager_item_layout"),
						parent, false);
			} catch (ResourceNotFoundException exp) {
				Log.e("ToolbarPreviewAdapter", "XML resouce not found!");
			}
				...
				try {
					padding = mContext
							...
							.getDimensionPixelSize(
									TiRHelper
											.getResource("dimen.toolbar_height")) - 4;
				} catch (ResourceNotFoundException exp) {
					Log.e("ToolbarPreviewAdapter", "XML resouce not found!");
				}
