...
public class MuPDFActivity extends Activity implements FilePicker.FilePickerSupport {
	...
	public void createAlertWaiter() {
		...
		mAlertTask = new AsyncTask<Void,Void,MuPDFAlert>() {
			...
			protected void onPostExecute(final MuPDFAlert result) {
				...
				{
				case OkCancel:
					mAlertDialog.setButton(AlertDialog.BUTTON2, getString(TiRHelper.getResource("string.cancel")), listener);
					pressed[1] = MuPDFAlert.ButtonPressed.Cancel;
				case Ok:
					mAlertDialog.setButton(AlertDialog.BUTTON1, getString(TiRHelper.getResource("string.okay")), listener);
					pressed[0] = MuPDFAlert.ButtonPressed.Ok;
					break;
				case YesNoCancel:
					mAlertDialog.setButton(AlertDialog.BUTTON3, getString(TiRHelper.getResource("string.cancel")), listener);
					pressed[2] = MuPDFAlert.ButtonPressed.Cancel;
				case YesNo:
					mAlertDialog.setButton(AlertDialog.BUTTON1, getString(TiRHelper.getResource("string.yes")), listener);
					pressed[0] = MuPDFAlert.ButtonPressed.Yes;
					mAlertDialog.setButton(AlertDialog.BUTTON2, getString(TiRHelper.getResource("string.no")), listener);
	...
	public void onCreate(Bundle savedInstanceState) {
			...
			if (Intent.ACTION_VIEW.equals(intent.getAction())) {
				...
				if (uri.toString().startsWith("content://")) {
					...
					if (reason != null) {
						...
						AlertDialog alert = mAlertBuilder.create();
						try {
						setTitle(String.format(res.getString(TiRHelper.getResource("string.cannot_open_document_Reason")), reason));
						alert.setButton(AlertDialog.BUTTON_POSITIVE, getString(TiRHelper.getResource("string.dismiss")),
		...
		if (core == null) {
			AlertDialog alert = mAlertBuilder.create();
			try {
			alert.setTitle(TiRHelper.getResource("string.cannot_open_document"));
			alert.setButton(AlertDialog.BUTTON_POSITIVE,
					getString(TiRHelper.getResource("string.dismiss")),
					new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							finish();
						}
		...
		switch (requestCode) {
			...
			case PRINT_REQUEST:
				if (resultCode == RESULT_CANCELED)
					try {
					showInfo(getString(TiRHelper.getResource("string.print_failed")));
					} catch (ResourceNotFoundException e ){}
				break;
	...
	*/
	...
	private void updatePageNumView(int index) {
		...
		if (core.getDisplayPages() == 2 && index!=0 && index!=core.countPages()-1) {
			...
			mPageNumberView.setText(String.format(getString(TiRHelper.getResource("string.two_pages_of_count")), (index*2)-2, (index*2)-1, core.countSinglePages()));
		}
		else if (core.getDisplayPages() == 2 && (index==0 || index==core.countPages()-1)) {
			pageStr = String.format("%1$d", index+1);
			mPageNumberView.setText(String.format(getString(TiRHelper.getResource("string.one_page_of_count")), index+1, core.countSinglePages()));
		}
		else {
			pageStr = String.format("%1$d", index+1);
			mPageNumberView.setText(String.format(getString(TiRHelper.getResource("string.one_page_of_count")), index+1, core.countPages()));
		}
		} catch( ResourceNotFoundException e ){}
	...
	private void printDoc() {
		if (!core.fileFormat().startsWith("PDF")) {
			try {
			showInfo(getString(TiRHelper.getResource("string.format_currently_not_supported")));
			} catch( ResourceNotFoundException e ){}
			return;
		}
	...
	private void showInfo(String message) {
		...
		if (currentApiVersion >= android.os.Build.VERSION_CODES.HONEYCOMB) {
			try {
			@SuppressWarnings("unused")
			SafeAnimatorInflater safe = new SafeAnimatorInflater((Activity)this, TiRHelper.getResource("anim.info"), (View)mInfoView);
			} catch( ResourceNotFoundException e ){}
		} else {
	...
	private void makeButtonsView() {
		try {
		mButtonsView = getLayoutInflater().inflate(TiRHelper.getResource("layout.buttons"), null);
		// ///////mFilenameView =
		// (TextView)mButtonsView.findViewById(R.id.docNameText);
		///mPageSlider = (SeekBar) mButtonsView.findViewById(R.id.pageSlider);
		mPreviewBarHolder = (FrameLayout) mButtonsView.findViewById(TiRHelper.getResource("id.PreviewBarHolder"));
		...
		try {		
		mPageNumberView = (TextView) mButtonsView.findViewById(TiRHelper.getResource("id.pageNumber"));
		mInfoView = (TextView) mButtonsView.findViewById(TiRHelper.getResource("id.info"));
		mSearchButton = (ImageButton) mButtonsView.findViewById(TiRHelper.getResource("id.searchButton"));
		...
		mTopBarSwitcher = (ViewAnimator) mButtonsView.findViewById(TiRHelper.getResource("id.switcher"));
		mSearchBack = (ImageButton) mButtonsView.findViewById(TiRHelper.getResource("id.searchBack"));
		mSearchFwd = (ImageButton) mButtonsView.findViewById(TiRHelper.getResource("id.searchForward"));
		mSearchText = (EditText) mButtonsView.findViewById(TiRHelper.getResource("id.searchText"));
	...
	*/
	...
	public void onBackPressed() {
		...
		if (core != null && core.hasChanges()) {
			...
			try {
			alert.setMessage(getString(TiRHelper.getResource("string.document_has_changes_save_them")));
			alert.setButton(AlertDialog.BUTTON_POSITIVE,
					getString(TiRHelper.getResource("string.yes")), listener);
			alert.setButton(AlertDialog.BUTTON_NEGATIVE,
					getString(TiRHelper.getResource("string.no")), listener);
			alert.show();
			}  catch( ResourceNotFoundException e ){}
		} else {
			super.onBackPressed();
