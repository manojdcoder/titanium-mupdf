...
public class TwoWayView extends AdapterView<ListAdapter> implements
	...
	public TwoWayView(Context context, AttributeSet attrs, int defStyle) {
		...
		try {
			int[] styles = new int[] { TiRHelper
					.getResource("styleable.TwoWayView") };
			TypedArray a = context.obtainStyledAttributes(attrs, styles,
					defStyle, 0);
			initializeScrollbars(a);

			mDrawSelectorOnTop = a
					.getBoolean(
							TiRHelper
									.getResource("styleable.TwoWayView_android_drawSelectorOnTop"),
							false);

			Drawable d = a.getDrawable(TiRHelper
					.getResource("styleable.TwoWayView_android_listSelector"));
			if (d != null) {
				setSelector(d);
			}

			int orientation = a.getInt(TiRHelper
					.getResource("styleable.TwoWayView_android_orientation"),
					-1);
			if (orientation >= 0) {
			...
			int choiceMode = a
					.getInt(TiRHelper
							.getResource("styleable.TwoWayView_android_choiceMode"),
							-1);
			if (choiceMode >= 0) {
			...
			Log.e("TwoWayView", "XML resouce not found!");
		}
