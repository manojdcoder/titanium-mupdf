...
public abstract class PageView extends ViewGroup {
	...
	// protected abstract Bitmap updatePage(BitmapHolder h, int sizeX, int
	// sizeY,
	// int patchX, int patchY, int patchWidth, int patchHeight);
	protected abstract void drawPage(Bitmap bm, int sizeX, int sizeY,
			int patchX, int patchY, int patchWidth, int patchHeight);

	protected abstract void updatePage(Bitmap bm, int sizeX, int sizeY,
			int patchX, int patchY, int patchWidth, int patchHeight);
	...
	public void blank(int page) {
		...
		if (mBusyIndicator == null) {
			...
			try {
				...
				Log.e("PageView", "XML resouce not found!");
			}
				...
				protected void onDraw(final Canvas canvas) {
					...
					if (!mIsBlank && mSearchBoxesPrim != null) {
						...
						for (RectF rect : mSearchBoxesPrim)
							canvas.drawRect(rect.left * scale + getWidth() / 2,
									rect.top * scale, rect.right * scale
											+ getWidth() / 2, rect.bottom
											* scale, paint);
					}
					...
					if (!mIsBlank && mLinks != null && mHighlightLinks) {
						...
						for (LinkInfo link : mLinks) {
							...
							// link.rect.top * scale,
							// link.rect.right * scale, link.rect.bottom *
							// scale, paint);
							RectF rectfa = new RectF((link.rect.left - 2)
									* scale, (link.rect.top - 2) * scale,
									(link.rect.right + 2) * scale,
									(link.rect.bottom + 2) * scale);
							paint.setStyle(Paint.Style.FILL);
							paint.setColor(LINK_COLOR);
							canvas.drawRoundRect(rectfa, 3 * scale, 3 * scale,
									paint);
